...
export const loginState = createReducer<LoginResponse>(initialState, {
  ...
  [SAVE_USER_INFO]: (state, action: ReturnType<typeof saveUserInfo>) => ({
    ...state,
    address: action.payload.address === 'null' ? null : action.payload.address,
    buildingCode:
      action.payload.address === 'null' ? null : action.payload.buildingCode,
    user_id: action.payload.user_id === 'null' ? null : action.payload.user_id,
  }),
